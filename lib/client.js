  ...
  factory: (require) => {
    ...
    const styles = {
      ...
      resizeHandle: {
        ...
        padding: 4, color: colors.textFaint, fontSize: 11, lineHeight: 1,
      },
      header: {
      ...
      iconBtn: {
        border: 'none', background: 'transparent', color: colors.textDim, cursor: 'pointer',
      ...
      dayBtn: {
        ...
        fontSize: 11, padding: '2px 8px', borderRadius: 999, cursor: 'pointer', fontFamily: 'inherit',
      },
      dayBtnActive: { color: colors.accent, borderColor: colors.accent, background: colors.accentBg },
      sortSel: {
      ...
      modal: {
        width: 640, maxWidth: '100%', maxHeight: '82vh', display: 'flex', flexDirection: 'column',
    ...
    function PluginCard({ item, onOpen, timeMode }) {
      const [hover, setHover] = useState(false)
      // 时间戳跟当前模式走：新建模式看 createdAt，更新模式看 pushedAt
      ...
      return h(
        ...
        h(
          ...
          isNew(stamp) && h('span', { style: styles.badgeNew }, 'NEW'),
          h(
            'span',
            {
    ...
    function VirtualList({ items, onOpen, timeMode }) {
      ...
      return h(
        ...
        slice.map((item) => h(PluginCard, { key: item.fullName, item, onOpen, timeMode })),
    ...
    function MarketPanel({ onClose, onOpen }) {
      const [data, setData] = useState({
        items: [], fetchedAt: 0, refreshing: false, lastError: '', pushWindowDays: 3,
      })
      // 面板尺寸：默认 400×620，用户可拖右下角手柄调整，localStorage 记住
      ...
      const onResizeStart = useCallback((e) => {
        ...
        const fromW = sizeRef.current.w
        const fromH = sizeRef.current.h
        const onMove = (ev) => {
          const w = Math.min(Math.max(320, fromW + (ev.clientX - startX)), Math.max(360, window.innerWidth - 48))
          const hh = Math.min(Math.max(320, fromH + (ev.clientY - startY)), Math.max(360, window.innerHeight - 120))
          setSize({ w, h: hh })
        }
        const onUp = () => {
          window.removeEventListener('pointermove', onMove)
          window.removeEventListener('pointerup', onUp)
          try { localStorage.setItem('plugin-radar-size', JSON.stringify(sizeRef.current)) } catch {}
        }
      ...
      return h(
        ...
        h(
          ...
          h(
            'button',
            { style: { ...styles.tab, ...(tab === 'feed' ? styles.tabActive : null) }, onClick: () => setTab('feed') },
            `最新 · ${feedCount}`,
          ),
          h(
            'button',
            { style: { ...styles.tab, ...(tab === 'all' ? styles.tabActive : null) }, onClick: () => setTab('all') },
          ...
          tab === 'feed' &&
            h(
              ...
              h(
                'button',
                {
                  style: { ...styles.dayBtn, ...(feedMode === 'created' ? styles.dayBtnActive : null) },
              ...
              h(
                'button',
                {
                  style: { ...styles.dayBtn, ...(feedMode === 'updated' ? styles.dayBtnActive : null) },
                  title: '近 N 天有代码提交的仓库（看活跃度）',
                  onClick: () => setFeedMode('updated'),
                },
                '🔄 更新',
              ),
              h('span', { style: { width: 4 } }),
              [3, 7].map((d) =>
                h(
                  'button',
                  {
        ...
        h(
          ...
          h(VirtualList, { items: filtered, onOpen, timeMode: tab === 'feed' ? feedMode : 'updated' }),
        ),
        h(
          'div',
          { style: styles.disclaimer },
        ...
        h(
          'div',
          { style: styles.resizeHandle, onPointerDown: onResizeStart, title: '拖动调整大小' },
          '⤡',
        ),
      )
    }
      ...
      const recount = useCallback(() => {
        fetch(`${API}/list`)
          ...
          .then((json) => {
            ...
            const cutoff = Date.now() - (json.pushWindowDays || 3) * 86400_000
        ...
        open &&
          h(
            'div',
            {
              style: {
                position: 'fixed',
                // 面板跟随 dock：水平超出右边界时向左展开，垂直始终在 dock 上方
                left: Math.min(pos.x, Math.max(8, window.innerWidth - 416)),
                bottom: Math.min(pos.y + 52, Math.max(120, window.innerHeight - 80)),
                zIndex: 998,
                pointerEvents: 'auto',
              },
            },
            h(MarketPanel, { onClose: () => setOpen(false), onOpen: setDetail, embedded: true }),
          ),
        dock,
        modal,
      )
