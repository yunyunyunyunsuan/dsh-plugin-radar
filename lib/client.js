...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
            ...
            h(
              ...
              it.isSelf
                ...
                : h(
                    'button',
                    {
                      style: { ...styles.installBtn, background: 'rgba(248,113,113,0.18)', color: colors.red, marginLeft: 'auto', opacity: busy === it.name ? 0.5 : 1 },
    ...
    function RecommendView({ items, onOpen }) {
      // 精选：按关键词在收录里找热门（换肤/IDE 界面/视觉/桌面等）
      const picks = useMemo(() => {
        const themes = [
          { tag: '界面美化 / 换肤', match: ['skin', 'theme', '主题', '换肤', '皮肤', 'ui-', '-ui', 'glass', 'fluent'] },
          { tag: 'IDE 化界面', match: ['ide', 'panel', '布局', 'workbench', 'editor', '分屏', 'tmux'] },
          { tag: '视觉方案', match: ['vision', '视觉', 'image', '图片', '看图', 'eyes', 'ocr'] },
          { tag: '桌面 / 启动器', match: ['desktop', '桌面', 'tray', 'launcher', '图标'] },
          { tag: '记忆 / 知识库', match: ['memory', '记忆', 'knowledge', '知识库', 'rag'] },
        ]
        const out = []
      ...
      return h(
        ...
        picks.map(({ tag, items: list }) =>
          h(
            ...
            list.map((it) => h(PluginCard, { key: it.fullName, item: it, onOpen, timeMode: 'updated', seenSet: null })),
          ),
        ),
        !picks.length && h('div', { style: styles.center }, '暂无推荐（等扫描收录更多插件后出现）'),
      )
    }
          ...
          data.refreshing
            ...
            : h(
                'a',
                {
                  ...
                  style: { color: colors.textFaint, fontSize: 11, textDecoration: 'none' },
                  title: '插件雷达开源仓库，觉得好用就点个 Star ⭐',
                },
                '⭐ 给插件雷达点个 Star',
              ),
          h('span', { style: { flex: 1 } }),
    ...
    function MarketWidget() {
      ...
      return h(
        ...
        h(
          ...
          // 面板（在胶囊上方；column-reverse 使视觉上面板在上、胶囊在下）
          open &&
            h(
              'div',
