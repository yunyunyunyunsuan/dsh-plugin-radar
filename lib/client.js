...
// 注入全局样式：彻底关闭所有按钮/链接/输入的点击后 focus 白边（Chromium/WebKit outline/focus ring 全清，键盘 Tab 在本面板非必要场景）
if (typeof document !== 'undefined' && !document.getElementById('plugin-radar-style')) {
  const tag = document.createElement('style')
  tag.id = 'plugin-radar-style'
  tag.textContent = `
    .plugin-radar-root *,
    .plugin-radar-root *::before,
    .plugin-radar-root *::after {
      outline: 0 !important;
    ...
    .plugin-radar-root button:focus,
    .plugin-radar-root button:focus-visible,
    .plugin-radar-root button:active,
    .plugin-radar-root button:hover,
    .plugin-radar-root a:focus,
    .plugin-radar-root a:focus-visible,
    .plugin-radar-root a:hover,
    .plugin-radar-root a:active,
    .plugin-radar-root input:focus,
    .plugin-radar-root select:focus,
    .plugin-radar-root textarea:focus {
      outline: 0 !important;
      outline-color: transparent !important;
      outline-width: 0 !important;
      outline-style: none !important;
    ...
    .plugin-radar-root button { -webkit-tap-highlight-color: transparent !important; }
  `
  document.head.appendChild(tag)
}

// 兜底：点击 button 后立即 blur 掉 active element，避免 macOS Chromium 给 button 留 focus ring 残影
if (typeof document !== 'undefined' && !document.getElementById('plugin-radar-blur-mouse')) {
  document.addEventListener('mouseup', (e) => {
    const t = e.target
    if (!t || !t.closest || !t.closest('.plugin-radar-root')) return
    setTimeout(() => {
      const ae = document.activeElement
      if (ae && ae !== document.body && ae.closest && ae.closest('.plugin-radar-root')) {
        try { ae.blur() } catch {}
      }
    }, 0)
  }, true)
  ...
  document.head.appendChild(m)
    ...
    const styles = {
      ...
      header: {
        ...
        borderBottom: `1px solid rgba(255,255,255,0.06)`, flex: 'none',
      },
      iconBtn: {
        ...
        justifyContent: 'center', fontSize: 16, flex: 'none',
      },
        ...
        h(
          ...
          h(
            ...
            h('button', { style: styles.iconBtn, onClick: onClose },
              h('svg', { width: 14, height: 14, viewBox: '0 0 16 16', fill: 'none', stroke: 'currentColor', strokeWidth: 1.7, strokeLinecap: 'round' },
    ...
    function MarketPanel({ onClose, onOpen, seenSet, onPanelDrag }) {
      ...
      const [size, setSize] = useState(() => {
        try {
          const s = JSON.parse(localStorage.getItem('plugin-radar-size-v2') || 'null')
          if (s && Number.isFinite(s.w) && Number.isFinite(s.h)) return s
        } catch {}
        // 默认按视口大小自适应：大屏 720×760，超宽 1000×760，中屏 540×660，窄屏 380×540
        const w = window.innerWidth >= 1600 ? 1000 : window.innerWidth >= 1280 ? 720 : window.innerWidth >= 800 ? 540 : 380
        const h = window.innerWidth >= 1280 ? 760 : window.innerWidth >= 800 ? 660 : 540
        return { w, h }
      })
      ...
      const startResize = useCallback((dir) => (e) => {
        ...
        const onUp = () => {
          ...
          try {
            localStorage.setItem('plugin-radar-size-v2', JSON.stringify({ w: sizeRef.current.w, h: sizeRef.current.h }))
          } catch {}
        }
      ...
      }, [base, tokens, sort, feedMode, minS, maxS])
      ...
      return h(
        'div',
        {
          ...
          // 窗口式拖动：面板任意空白处按住即拖走整个界面（按钮/输入/卡片/链接/缩放条不触发）
          onPointerDown: (e) => {
            const t = e.target
            const interactive = t.closest && t.closest('button, input, select, a, textarea, [role="option"], [style*="ns-resize"], [style*="ew-resize"], [style*="nwse-resize"], [style*="nesw-resize"]')
            if (interactive) return
            // 滚动容器内也允许（卡片流区域不拖），只允许 header/tab/空白区拖
            if (t.closest && t.closest('[data-no-drag]')) return
            onPanelDrag && onPanelDrag(e)
          },
        },
        // 四边 + 四角拖拽条
        ...
        h(
          'div',
          {
            style: { ...styles.header, padding: '10px 14px 6px', cursor: 'grab' },
            onPointerDown: (e) => {
              // 按钮/链接/输入不触发拖动
          ...
          h(
            ...
            // 刷新图标（rotation arrow）—— 14×14 让两图标视觉等大
            h('svg', { width: 14, height: 14, viewBox: '0 0 16 16', fill: 'none', stroke: 'currentColor', strokeWidth: 1.7, strokeLinecap: 'round', strokeLinejoin: 'round' },
              h('path', { d: 'M13.5 8a5.5 5.5 0 1 1-1.6-3.9' }),
              h('polyline', { points: '14 2.5 14 5 11.5 5' }),
            ),
          ),
          h('button', { style: styles.iconBtn, title: '收起', onClick: onClose },
            // 关闭图标（✕）—— 线条扩到 4→12（占 viewBox 50%，Lucide 标准比例），跟 ⟳ 视觉等大
            h('svg', { width: 14, height: 14, viewBox: '0 0 16 16', fill: 'none', stroke: 'currentColor', strokeWidth: 1.7, strokeLinecap: 'round' },
              h('line', { x1: 4, y1: 4, x2: 12, y2: 12 }),
              h('line', { x1: 12, y1: 4, x2: 4, y2: 12 }),
            ),
        ...
        h(
          'div',
          // 整体网格：tab 列（左）+ 工具列（右），工具永远靠右；空白处也是拖动把手
          {
            style: {
              ...styles.tabBar,
              display: 'grid',
          ...
          h(
            ...
            h(
              ...
              `已安装${installedCount !== null ? ` · ${installedCount}` : ''}`,
            ),
            h(
              'button',
          ...
          // 右列：信息流工具（仅 feed/all）；单独列永远靠右
          (tab === 'feed' || tab === 'all') && h(
            'div',
            { style: { display: 'flex', gap: 4, alignItems: 'center', justifyContent: 'flex-end', flexWrap: 'wrap' } },
            h(
              'button',
              {
                key: 'new',
            ...
            h('span', { key: 'star', style: { fontSize: 11, color: colors.textFaint, marginLeft: 4 } }, '★'),
            h('input', {
              key: 'min', style: { ...styles.starInput, width: 40 }, placeholder: '0', value: minStars, type: 'number', min: 0,
              onChange: (e) => setMinStars(e.target.value), title: '最低星数（留空不限）',
            }),
            h('span', { key: 'sep', style: { fontSize: 11, color: colors.textFaint } }, '–'),
            h('input', {
              key: 'max', style: { ...styles.starInput, width: 40 }, placeholder: '∞', value: maxStars, type: 'number', min: 0,
        ...
        h(
          'div',
          { style: styles.list, 'data-no-drag': true },
        ...
        h(
          ...
          open &&
            h(
              ...
              h(MarketPanel, { onClose: () => setOpen(false), onOpen: openDetail, seenSet: seen, onPanelDrag: onPointerDown }),
            ),
          // 胶囊（把手）贴在面板正下方
