      ...
      return h(
        'div',
        {
          ...
          style: {
            ...
            // 面板底部对齐胶囊顶部（胶囊高 ~44px + 8px 间距），始终贴在它正上方
            left: Math.min(anchor.x, Math.max(8, window.innerWidth - size.w - 16)),
            top: Math.max(8, window.innerHeight - anchor.y - 52 - Math.min(size.h, Math.max(360, window.innerHeight - 120))),
          },
        },
        // 四边 + 四角拖拽条
        ...
        h(
          ...
          h(
            ...
            data.refreshing ? '正在扫描 GitHub（约 1-3 分钟）…' : `${data.items.length} 个插件`,
          ),
          h('span', { style: { flex: 1 } }),
          h(
            'button',
        ...
        h(
          ...
          // 统一的工具组（两个 tab 同步显示）：新建/更新仓库 + 星数筛选 + 排序
          h(
            'span',
