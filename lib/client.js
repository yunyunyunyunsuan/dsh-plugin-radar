  ...
  factory: (require) => {
    ...
    function MarketPanel({ onClose, onOpen, seenSet }) {
      ...
      return h(
        ...
        h(
          ...
          h(
            'button',
            {
              style: {
                ...styles.chipBase,
                ...(onlyInstallable ? styles.chipActive : null),
                marginRight: 6,
              },
        ...
        h(
          'div',
          // 单一可换行容器，所有 chip 直接放同一层 flex 流（够宽时单行，窄了自然换行）
          {
            style: {
              ...styles.tabBar,
              display: 'flex', flexDirection: 'row', flexWrap: 'wrap',
              alignItems: 'center', gap: '6px 6px',
            },
          },
          // 四个 tab
          h(
          ...
          // 把工具组推到行尾（tab 4 个占左，工具组靠右）；窄时自然换行
          (tab === 'feed' || tab === 'all') && h('div', { key: 'spacer', style: { flex: '1 0 0', minWidth: 0 } }),
          (tab === 'feed' || tab === 'all') && [
            h(
              'button',
              {
