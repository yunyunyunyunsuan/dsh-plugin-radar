...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const styles = {
      pill: {
        position: 'fixed', left: 16, bottom: 16, zIndex: 999, pointerEvents: 'auto',
      ...
      panel: {
        position: 'fixed', left: 16, bottom: 16, zIndex: 999, pointerEvents: 'auto',
        width: 400, maxWidth: 'calc(100vw - 32px)', height: 'min(620px, calc(100vh - 96px))',
      ...
      overlay: {
        position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(8, 9, 13, 0.6)',
    ...
    function PluginCard({ item, onOpen }) {
      ...
      return h(
        ...
        h(
          ...
          h('span', { style: { ...styles.meta, cursor: 'default' } }, '点卡片看详情'),
    ...
    function MarketPanel({ onClose, onOpen }) {
      ...
      return h(
        ...
        h(
          ...
          filtered.map((item) => h(PluginCard, { key: item.fullName, item, onOpen })),
        ),
        h(
          'div',
          { style: styles.disclaimer },
    ...
    function MarketWidget() {
      ...
      return h(
        ...
        null,
        open
          ? h(MarketPanel, { onClose: () => setOpen(false), onOpen: setDetail })
          : h(
              'button',
        ...
        detail && h(DetailModal, { item: detail, onClose: () => setDetail(null) }),
      )
    }
