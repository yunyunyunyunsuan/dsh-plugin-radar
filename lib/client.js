  ...
  factory: (require) => {
    ...
    function MarketPanel({ onClose, onOpen, seenSet }) {
      ...
      const [size, setSize] = useState(() => {
        ...
        // 默认按视口大小自适应：宽屏 560×680，中屏 480×620，窄屏 380×540
        const w = window.innerWidth >= 1280 ? 560 : window.innerWidth >= 800 ? 480 : 380
        const h = window.innerWidth >= 1280 ? 680 : window.innerWidth >= 800 ? 620 : 540
        return { w, h }
      })
      ...
      return h(
        ...
        h(
          'div',
          { style: { ...styles.tabBar, display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'stretch' } },
          // 第一行：四个 tab + 可安装
          h(
            'div',
            ...
            h(
              'button',
              {
                style: { ...styles.chipBase, marginLeft: 'auto', ...(onlyInstallable ? styles.chipActive : null) },
          ...
          (tab === 'feed' || tab === 'all') &&
            h(
              ...
              h('span', { style: { fontSize: 11, color: colors.textFaint, marginLeft: 4 } }, '★'),
              h('input', {
                style: { ...styles.starInput, width: 40 }, placeholder: '0', value: minStars, type: 'number', min: 0,
                onChange: (e) => setMinStars(e.target.value), title: '最低星数（留空不限）',
              }),
              h('span', { style: { fontSize: 11, color: colors.textFaint } }, '–'),
              h('input', {
                style: { ...styles.starInput, width: 40 }, placeholder: '∞', value: maxStars, type: 'number', min: 0,
