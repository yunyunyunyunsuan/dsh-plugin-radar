...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const styles = {
      ...
      panel: {
        position: 'fixed', left: 28, bottom: 116, zIndex: 998, pointerEvents: 'auto',
        width: 400, maxWidth: 'calc(100vw - 32px)', height: 'min(620px, calc(100vh - 120px))',
        display: 'flex', flexDirection: 'column', borderRadius: '16px 16px 0 0',
        border: `1px solid ${colors.border}`, borderBottom: 'none', background: colors.bg,
    ...
    function PluginCard({ item, onOpen }) {
      ...
      return h(
        ...
        h(
          ...
          h('span', { style: styles.meta, title: absTime(item.pushedAt) }, `更新于 ${relTime(item.pushedAt)}`),
        ),
    ...
    function VirtualList({ items, onOpen }) {
      ...
      return h(
        ...
        slice.map((item) => h(PluginCard, { key: item.fullName, item, onOpen })),
    ...
    function MarketPanel({ onClose, onOpen }) {
      ...
      return h(
        ...
        h(
          ...
          h(VirtualList, { items: filtered, onOpen }),
        ),
        h(
          'div',
          { style: styles.disclaimer },
    ...
    function MarketWidget() {
      ...
      const dock = h(
        'button',
        {
          style: { ...styles.dock, ...(open ? { background: colors.accentBg, borderColor: colors.accent } : null) },
          onClick: toggle,
          title: open ? '收起' : '展开插件雷达',
        },
        h('span', null, '🧩'),
        h('span', null, '插件雷达'),
        !open && newCount > 0 &&
      ...
      return h(
        React.Fragment,
        null,
        open && h(MarketPanel, { onClose: () => setOpen(false), onOpen: setDetail }),
        dock,
        modal,
      )
    }
