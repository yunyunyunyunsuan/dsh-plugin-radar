          ...
          try {
            ...
            const enriched = list.map((it) => {
              const hit = byName.get(it.name.toLowerCase()) ||
                byName.get(it.name.split('/').pop().toLowerCase())
              return { ...it, summary: hit?.summary || '', fullName: hit?.fullName || '', stars: hit?.stars ?? null, isSelf: it.name.toLowerCase() === 'dsh-plugin-radar' }
            })
