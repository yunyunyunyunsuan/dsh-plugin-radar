...
export function apply(ctx, config = {}) {
  const cfg = {
    ...
    pushWindowDays: 3,
    deepScanDays: 3,
    maxRepos: 0, // 0 = 不限，受 GitHub API 物理上限 5819
    pagesPerSort: 30,
    pkgProbePerScan: 60,
    readmeMaxChars: 6000,
  ...
  /** 首次启动无缓存时加载包内种子数据（开箱即见全量列表），后台再增量更新 */
  async function loadSeed() {
    if (state.items.length) return
    try {
      const seedFile = path.join(path.dirname(new URL(import.meta.url).pathname), 'seed.json')
      const data = JSON.parse(await fsp.readFile(seedFile, 'utf8'))
      if (Array.isArray(data.items) && data.items.length) {
        state.items = data.items
        state.fetchedAt = data.fetchedAt || Date.now()
        if (data.scanCursor?.lastCreatedDay) scanCursor.lastCreatedDay = data.scanCursor.lastCreatedDay
        log(`seed loaded: ${state.items.length} plugins (built ${new Date(state.fetchedAt).toISOString().slice(0, 10)})`)
      }
    } catch {
      /* 包内无种子，首次扫描填充 */
    }
  ...
  /** 探测仓库是否有 package.json（可否一键安装的 JS 插件） */
  async function detectPkg(fullName) {
    try {
  ...
  async function scan() {
    ...
    try {
      ...
      for (let d = new Date(start); d <= yesterday; d.setUTCDate(d.getUTCDate() + 1)) {
        ...
        if (cfg.maxRepos > 0 && byName.size >= cfg.maxRepos) { log('达到 maxRepos，停'); break }
        await sleep(SEARCH_GAP_MS)
      }
      ...
      if (cfg.maxRepos > 0 && allRepos.length > cfg.maxRepos) {
      ...
      for (const raw of allRepos) {
        ...
        const base = {
          ...
          summary: '',
          deep: false,
          hasPkg: null, // 是否有 package.json（可否一键安装），深收时探测
        }
        if (reuse) {
          base.readme = prev.readme
          base.summary = prev.summary
          base.hasPkg = prev.hasPkg ?? null
      ...
      // 只有近窗口内、且没缓存的才抓 README + 探测 package.json
      ...
      const workers = Array.from({ length: 4 }, async () => {
        while (queue.length) {
          ...
          base.hasPkg = await detectPkg(repo.full_name)
        }
      })
      await Promise.all(workers)
      // 老条目惰性补探测 hasPkg：每次扫描限量（窗口内优先）
      const pending = items.filter((it) => it.hasPkg === null).slice(0, cfg.pkgProbePerScan)
      if (pending.length) {
        const pq = [...pending]
        await Promise.all(
          Array.from({ length: 4 }, async () => {
            while (pq.length) {
              const it = pq.shift()
              it.hasPkg = await detectPkg(it.fullName)
            }
          }),
        )
        log(`pkg probe backfill: ${pending.length} (剩余未探测 ${items.filter((i) => i.hasPkg === null).length})`)
      }
      // 按更新时间倒序落库（最新在前）
  ...
  async function installedSet() {
    ...
    try {
      ...
      for (const b of pkg.dsh?.profile?.bundles || []) set.add(String(b).toLowerCase())
    } catch {}
    try {
  ...
  /** 判断一个 patch.yml insert 块是否指向指定包：按 name（兼容 'x'/"x"/x 三种写法）或 id 匹配 */
  function blockTargets(blockText, packageName) {
    const lower = packageName.toLowerCase()
    const attr = (key) => {
      const m = blockText.match(new RegExp(`(?:^|\\n)\\s*${key}:\\s*(?:'([^']+)'|"([^"]+)"|([^\\s#'"][^#'\\n]*))`, 'i'))
      if (!m) return ''
      return (m[1] || m[2] || m[3] || '').replace(/['"]/g, '').trim().toLowerCase()
    }
    return attr('name') === lower || attr('id') === lower
  }

  /** 一键卸载：从 patch.yml 摘除启用行 + 移出 bundles + pnpm remove */
  async function uninstall(packageName) {
    ...
    // ① 从 patch.yml 删掉 name/id 指向该包的 insert 块（兼容带引号/不带引号）
    try {
      const yml = await fsp.readFile(patchFile, 'utf8')
    ...
    // ② 从 dsh.profile.bundles 摘掉（bundle 型插件）
    try {
      const pkgJson = JSON.parse(await fsp.readFile(profilePkgFile, 'utf8'))
      const bundles = pkgJson.dsh?.profile?.bundles || []
      if (bundles.some((b) => String(b).toLowerCase() === lower)) {
        pkgJson.dsh.profile.bundles = bundles.filter((b) => String(b).toLowerCase() !== lower)
        await fsp.writeFile(profilePkgFile, JSON.stringify(pkgJson, null, 2), 'utf8')
        log(`removed ${packageName} from dsh.profile.bundles`)
      }
    } catch (e) {
      log(`bundles clean failed: ${e.message}`)
    }
  ...
  ctx.effect(() => {
    ...
    void (async () => {
      await loadCache()
      await loadSeed() // 无缓存时加载包内种子，开箱即见全量列表
