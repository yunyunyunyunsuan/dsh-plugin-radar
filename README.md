...
- **0.4.13** — 修复「一键卸载没作用」：卸载时移除 `cordis.patch.yml` 里 `name:` 带/不带引号的 insert 行（此前只匹配带引号写法，导致如 `name: dsh-plugin-moments` 这类行删不掉、插件卸载后仍残留）；新增从 `dsh.profile.bundles` 摘除 bundle 型插件；`installedSet` 现识别 bundle 型插件，使其出现在「已安装」里可被卸载
- **0.4.12** — 种子数据更新至 7036 插件（开箱即见全量）；窗口式拖动（header / tab 行空白处按住即拖动整个界面）；修 Chrome 下按钮点击后残留 focus 白边；⟳/✕ 图标统一为等大 SVG
